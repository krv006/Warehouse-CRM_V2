...
| GET | `/configuration-requests/{id}/roadmap/` | B8: **zanjir ko'zgusi** — 20 qadam (20-§3.5: `act_review` qo'shildi), uch kirish nuqtasi bir xil javob; barcha rollar to'liq ko'radi, **pul yo'q**; `label`/`state`/`tone`/`waiting_days`/`deadline`/`can_open`/`repeats`/`current_key` tayyor keladi |
...
| POST | `/configuration-requests/{id}/reject/` | B15/9-§1A: **engineer** FAQAT `new` zayavkani izoh bilan qaytaradi (ishga olinganida 400 — `ask-sales` yoki `release` bor); `returned` — faqat sales'da (hovuzdan chiqadi). 16-§A2: SAVDODAGI modellar **sales ko'rigida** (`pending_sales`) bo'lsa — bitta manzil endi ENDI shu holatni ham biladi: **sales** izoh bilan barcha shu bosqichdagi modellarni `draft`ga qaytaradi (izoh majburiy) |
...
| POST | `/configuration-requests/{id}/submit/` | 16-§A2: **engineer** — savdodagi barcha `draft` modellarni ko'rikka yuboradi. **Hammasi yoki hech nima**: biror model tayyor bo'lmasa (tarkib bo'sh/narxsiz/allaqachon boshqa bosqichda) — 400, hech biri o'zgarmaydi: `{"detail": "...", "blocked": [{"id","number","reason","message"}]}` (`reason`: `no_items`/`needs_price`/`wrong_status`). Bitta modelli ZVKda model darajasidagi `submit` bilan bir xil natija |
| POST | `/configuration-requests/{id}/approve/` | 16-§A2: **sales** — savdodagi barcha `pending_sales` modellarni tasdiqlaydi; har biri o'z `approve`sidan o'tgani uchun 14-§3 dagi avtomatik birlashtirish shu bilan **bitta shartnomani** o'zi kafolatlaydi. `separate_contract` bu yerda **qabul qilinmaydi** |
| POST | `/configuration-requests/{id}/ask-sales/` | 16-§A3: **engineer** — savol **asosiy modelga** yoziladi (bitta suhbat), lekin savdodagi BARCHA `draft` modellar `pending_clarification`ga o'tadi — javobsiz turganda ikkinchi model ustida ishlashning ma'nosi yo'q (`submit` baribir bloklaydi) |
| POST | `/configuration-requests/{id}/answer/` | 16-§A3: **sales** — teskarisi, barcha `pending_clarification` modellarni `draft`ga qaytaradi |
| POST | `/configuration-requests/{id}/request-prices/` | 16-§A2: **engineer** — savdodagi BARCHA modellarning narxsiz qatorlarini yig'ib, buyurtmachiga bitta to'plam qilib yuboradi; javobda qaysi model uchun so'ralgani ham bor: `{"requested": [{"product","name","configurations": ["CFG-...", ...]}]}` |
| POST | `/configuration-requests/{id}/assemble/` | 16-§A2: **engineer** — savdodagi barcha `approved` modellarni ketma-ket yig'adi. **A0: bu MEHNAT — atomar emas**, natija model-model qaytadi: `{"assembled": ["CFG-..."], "pending": [{"number","missing": [...]}]}`; bitta model yig'ilsa ham `200`, `400` faqat HECH BIRI yig'ilmaganda |
| POST | `/configuration-requests/{id}/finalize/` | 16-§A2: **engineer** — savdodagi barcha YIG'ILGAN modellarni yakunlaydi, tanadagi `{"act": id}` HAMMASIGA bitta ACT sifatida biriktiriladi; javob: `{"finalized": [...], "pending": [...], "contract": {...}|null}` — yig'ilmagan model `pending`da qoladi, yakunlanmaydi |
| POST | `/configuration-requests/{id}/lines/` | 16-§B2: **sales (egasi)** (admin) — savdo boshlangandan keyin YANGI model/tovar qo'shadi (bugungacha `lines[]` faqat yaratishda o'qilardi). Tana: `{"kind": "model"\|"item", "base_product": id, "quantity": N, "text": "...", "mode": "build"}`; zayavka `in_progress`+ bo'lsa MODEL turiga darhol chernovik ochiladi (javobda `configuration`), `new` bo'lsa `take`gacha kutadi (`configuration: null`); shartnoma `pending_bugalter`+ bo'lsa **400** (`contract`, `contract_status` bilan); `cancelled`/`archived` zayavkada ham 400 |
...
`awaiting_balance`, `review_act`, `in_progress`). `level`:
`info`/`warning`/`danger` — danger birinchi turadi. **19-§2**:
`awaiting_balance` — yetkazilgan-u qoldiq to'lanmagan `active` shartnoma,
bugalter navbatida (yo'l xaritasidagi "Qoldiq to'lov" bilan bir xil
shart). **20-§3.7**: `review_act` — bugalter navbatidagi `pending_bugalter`
ACT.
...
| Metod | Manzil | Izoh |
|---|---|---|
| GET/POST | `/acts/` | yozish **engineer** (admin) — §11.1: ACT tarkib egasida. 20-§3: yaratilganda `status=draft`; javobda `status`/`status_display`/`approvals[]` (`ActApproval` tarixi) |
| POST | `/acts/{id}/submit/` | 20-§3.3: **engineer** (admin) — bugalter tasdig'iga yuboradi; faqat `draft`/`rejected`dan |
| POST | `/acts/{id}/approve/` | 20-§3.3: **bugalter** (admin) — tasdiqlaydi, `approved`; faqat `pending_bugalter`dan; shartnoma egasi sales'ga "yetkazish ochildi" eslatmasi |
| POST | `/acts/{id}/reject/` | 20-§3.3: **bugalter** (admin) — izoh MAJBURIY, `rejected`; faqat `pending_bugalter`dan; engineer (`created_by`)ga eslatma |
...
| POST | `/configurations/{id}/detach/` | 14-§5/16-§B5: **sales (egasi)** (admin) — ko'p modelli savdodan bitta modelni ajratadi, butun zanjirni bekor qilmaydi; tana `{"reason": "...", "target": "cancel"\|"separate"}`; `cancel` — konfiguratsiya `cancelled`, bron bo'shaydi; `separate` — model o'z alohida **yangi qoralama shartnomasi**ga ko'chadi; ikkalasida ham umumiy shartnomadan qator olib tashlanadi, `total_amount` qayta hisoblanadi; faqat shartnoma `draft` bo'lganda (aks holda 400 — yuborilgan/pul kelgan shartnomadan model olib tashlanmaydi). 16-§B5: javobda `warnings[]` — TLD hisobi tasdiq yo'lida bo'lsa qatorlari QOLADI (mol baribir keladi) va shu yerda aytiladi, yig'ilgan bo'lsa (`assembled_at`) variant omborda qolgani haqida ogohlantiradi; **ASOSIY model** chiqarilsa savdodagi eng eski tirik model avtomatik asosiy bo'lib ko'tariladi (`promoted_primary`) — tirik model umuman qolmasa 400 (`cancel` bilan butun savdo yopilsin) |
...
| GET/DELETE | `/configuration-request-lines/{id}/` | 16-§B5(f): faqat TOVAR (`kind=item`) qatorini savdodan olib tashlash — model turidagi qator (`kind=model`) `configurations/{id}/detach/` orqali chiqadi, bu yerda 400. `DELETE` — sales (egasi)/admin; bog'liq `ContractItem` bo'lsa birga o'chadi, shartnoma `total_amount` qayta hisoblanadi; shartnoma `draft` emas bo'lsa 400 |
...
bilan AYNAN bir xil (20 qadam, pul yo'q, `can_open` alohida). Tartib:
...
adashtirilmasin). `steps` doim 20 ta va tartibda (20-§3.5: `finalize`
bilan `ship` orasiga `act_review` — bugalter ACT ni tasdiqlamaguncha
`ship` qulflangan; konfiguratsiyasiz shartnomada bu qadam `skipped`);
`repeats` — aylanma
...
| GET/POST | `/contracts/` | admin, sales; filtr: `status`, `client`, `currency`, `configuration`; 8-to'plam §3: `configuration` bilan ikkinchi shartnoma ochilmaydi (400 — yangi oqimda u tasdiqda avtomatik ochiladi); qo'lda tuzish ombordan to'g'ridan-to'g'ri sotuv uchun. **20-§3.4**: javobda `acts_approved` (bool) — qator ortidagi ACTlarning hammasi `approved`mi, front yetkazish tugmasini qo'shimcha so'rovsiz o'chira oladi |
...
| POST | `/contracts/{id}/reject/` | bugalter / admin. 20-§2: tanadagi `target` — `sales` (standart, chernovikka) yoki `bugalter` (**faqat admin**, **faqat `pending_admin`dan** — hujjatdagi xatoni to'g'ridan bugalterga qaytaradi, holat `pending_bugalter`); `pending_bugalter`dan `target=bugalter` — 400 ("allaqachon bugalterda"); noma'lum `target` — 400; tarixda `ContractApproval.returned_to` (`''`/`'bugalter'`) |
| POST | `/contracts/{id}/ship/` | #2: **yetkazish** — mol shu yerda chiqadi; 11-§3: **sales (shartnoma egasi) va admin** — buyurtmachi/bugalter endi bosolmaydi (mol bilan ishlaydigan emas, mijoz bilan gaplashadigan odam yetkazadi); `delivered_at`/`delivered_by` yoziladi, bron chiqimga aylanadi, balans yopiq bo'lsa `completed`. **20-§3.4**: qator ortidagi konfiguratsiyaning ACT'i `approved` bo'lmasa **400** — `{"detail": "...", "acts": [{"id","number","status"}, ...]}`, mol chiqmaydi, bron turadi; konfiguratsiyasiz qatorlarda (tayyor tovar) bu tekshiruv yo'q |
