...
| `apps/sales/tests/test_contract_document.py` | 21-§3.2/3.6: GET lazily hujjat ochadi, engineer/buyurtmachiga yopiq (404 — queryset), sales (egasi)/bugalter/admin `can_edit`, begona sales 404; PATCH bilan matn to'g'ridan-to'g'ri saqlanadi (yangi — `.docx`/Collabora/WOPI olib tashlandi), Didoxdan keyin 400; shablon biriktirish (`attach-template`) matnni butunlay almashtiradi, nofaol shablon 400; `submit_contract` matnsiz 400; `ContractTemplateTests` — sales/admin yozadi, bugalter faqat o'qiydi, noma'lum `{{ }}` kalit 400, ikkinchi `is_default` birinchisini o'chiradi |
| `apps/sales/tests/test_contract_pdf_export.py` | 21-§3.3/3.7: barcha ma'lum kalitlar almashishi, noma'lum kalit tegilmasligi, spetsifikatsiya jadvali (ИТОГО + summa so'z bilan), rekvizit bloki mijoz turiga qarab shoxlanishi, Quill sinflari saqlashda inline uslubga ko'chirilishi, `render_contract_pdf_html` ramka HTML'i, PDF eksport endpoint huquqi (WeasyPrint mock — bu depozitoriy Windows sandbox'da native GTK/Pango kutubxonasisiz ishlaydi, productionda Linux paketlar bilan haqiqiy PDF chiqadi) |
| `apps/sales/tests/test_contract_number.py` | 21-§3.5: `NB2309-26` formati — to'liq ism, Kirill transliteratsiya, `username` zaxirasi, egasiz `XX`, bitta so'zli ism; takror raqamga `/2`/`/3`; eski `SHT-000xx` o'zgarmasligi, avtomatik ochilgan shartnoma raqami zayavka egasidan (finalizer'dan emas) |
