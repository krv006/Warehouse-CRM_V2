...
- **12-§2 (B)**: bitta zayavkada bir nechta talab — `ConfigurationRequestLine` (birinchi model `ConfigurationRequest.base_product`da qoladi, qolgani shu qatorlarda); ikki turi: `model` (yig'iladigan, `take`da chernovik ochadi) va `item` (tayyor tovar, konfiguratorsiz — shartnoma ochilganda to'g'ridan `ContractItem`ga aylanadi). Zayavka `ConfigurationRequest.is_fully_done` bo'lguncha (barcha qatori tugaguncha) `done` bo'lmaydi
